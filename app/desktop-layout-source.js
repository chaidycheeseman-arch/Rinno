...
async function saveCurrentLayout(options = {}) {
    purgeEmptySlots();
    repairLooseAppIcons();
    normalizeFreeIconGrids();
    purgeEmptyFreeIconGrids();
...
function checkEmptyPages() {
    purgeEmptySlots();
    purgeEmptyFreeIconGrids();
    if (getDesktopPages().length === 0) createNewPage();
    clampCurrentPageToPages();
    updatePagination();
    syncSliderToCurrentPage();
}
...
function moveArrange(e) {
    ...
    if (ghostNode && draggedNode) {
        ...
        const screenY = clampNumber(point.y, 0, viewport.height - 1);
        ...
        const maxY = Math.max(0, viewport.height - ghostNode.offsetHeight);
