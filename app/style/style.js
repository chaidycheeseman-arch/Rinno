...
function applyStyleWallpaper() {
    ...
    root.style.setProperty('--style-lock-wallpaper-size', styleState.lockWallpaper ? 'auto, cover' : 'cover');
...
function applyStyleAppIconCoverToElement(app, cover = '') {
    ...
    app.classList.toggle('style-icon-cover', Boolean(normalizedCover));
    if (!icon) {
        if (!normalizedCover) app.style.removeProperty('--style-app-icon-cover');
        return;
    }
    if (normalizedCover) {
