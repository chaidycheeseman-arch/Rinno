...
function buildPrivateContactChatPlaceholderContext(contact = null) {
    ...
    return {
        charName: String(activeContact?.title || activeContact?.name || 'TA').trim() || 'TA',
...
function buildPrivateContactChatPromptLiveState(thread = {}) {
    ...
    const gapMinutes = latestVisible?.createdAt
        ...
        : 0;
    return {
        currentTimeText: new Date().toLocaleString('zh-CN', {
            dateStyle: 'full',
            timeStyle: 'short'
        }),
...
function buildPrivateContactChatPromptContext(contact = {}, thread = {}, chat = {}, history = [], prologueBlocks = {}) {
    ...
    const totalHistoryMessageCount = normalizePrivateContactChatMessages(thread?.messages)
        .filter(message => ['user', 'assistant'].includes(String(message?.role || '')))
        .length;
    const liveState = buildPrivateContactChatPromptLiveState(thread);
    return {
        userName: userProfile.name,
        userId: userProfile.id,
...
function buildPrivateContactChatAssistantSystemPrompt(contact = {}, promptContext = {}) {
    ...
    return [
        ...
        '14. 同一话题连续承接超过 3 次后，自然换气，可以更短、更轻、更像真人停顿，也可以回头补一条刚想到的话，不要一直满格输出',
        '15. 遇到亲密、暧昧或高情绪话题时，也要保持角色内表达和沉浸感，不要跳成系统说明',
        '16. 如果环境里出现序章词条，必须把 before / middle / after 当作前置、过程、收束三段潜在约束，不要混写',
        '17. 只输出严格 JSON 数组，不要 Markdown，不要解释，不要前后缀',
        `18. 顶层只能是 JSON 数组；至少 ${PRIVATE_CONTACT_CHAT_MIN_PLAN_LENGTH} 个元素，条数由当下情绪、关系和内容自然决定，但不能少于 ${PRIVATE_CONTACT_CHAT_MIN_PLAN_LENGTH} 条`,
        '19. 数组内每一项只能是带 type 字段的 JSON 对象，不能输出字符串项、嵌套数组、自然语言说明或额外包装对象',
        '20. 联系人回复语言只能由国籍决定，不能跟着用户语言切换',
        '21. 不要输出空数组、未知字段、空 content 或 type 之外的动作名',
        `22. content 必须使用${languageSpec.language}，保持角色口吻，不要代替用户发言，也不要把输入占位、输入框文案或界面提示写进回复`,
        languageSpec.translationRequired
            ? '23. translation_zh 必须逐条对应 content，只写简体中文译文，不添加解释、括号、标签或“翻译：”。'
            : '23. 不需要也不要生成隐藏翻译字段。',
        '24. camera 只能写 description；voice 的 content 是语音转文字；duration_seconds 是 2 到 60 的数字；location 必须写 name，可选 address 和 note',
        '25. sticker 只能从 <sticker_inventory> 里精确选取，并且优先输出 {"type":"sticker","sticker_ref":"S01"}；一旦你决定发表情，就必须输出 sticker 对象，不要把“委屈猫猫”“抱抱小狗”这类库存描述词当成 text 正文发出去',
        '26. Rinno 富消息总量规则：每一轮最多只能出现 1 条富消息；sticker 可以自然出现，但不要一轮连发多张；voice、camera、location 依旧保持低频',
        '27. 如果 <sticker_inventory availability="disabled">，视为 sticker 功能不可用，直接改用 text',
        '</interaction_logic>',
        '',
        '<action_library>',
...
function switchPrivateTab(name, announce = false) {
    const target = ['whisper', 'contacts', 'time', 'monologue', 'sticker-library', 'contact-homepage', 'contact-chat'].includes(name) ? name : 'whisper';
    const chatScreen = document.querySelector('.private-chat-screen');
    if (chatScreen) chatScreen.setAttribute('data-private-current-tab', target);
    const primaryTab = target === 'contact-chat'
